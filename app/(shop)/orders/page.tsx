...
import PaymentProofModal from "@/components/features/orders/PaymentProofModal"; // Import Modal Payment
...
export default function UserOrdersPage() {
  ...
  // Update Type Modal agar support 'payment'
  const [modalType, setModalType] = useState<"tracking" | "rating" | "payment" | null>(null);
  ...
  // FILTER DATA (Tambah 'verifikasi' ke active)
  const filteredOrders = orders.filter(order => {
    if (activeTab === "active") {
      return ["pending", "verifikasi", "proses", "dikirim"].includes(order.status);
    } else {
  ...
  const getStatusBadge = (status: string) => {
    switch(status) {
      ...
      case "verifikasi": return <span className="text-xs font-bold text-orange-500 bg-orange-50 px-2 py-1 rounded">Menunggu Verifikasi</span>;
                  ...
                  <>
                    ...
                    {order.status === "pending" ? (
                        <Button 
                          ...
                          onClick={() => { setSelectedOrder(order); setModalType("payment"); }}
                        >
                          Konfirmasi Bayar
                        </Button>
                    ) : order.status === "verifikasi" ? (
                        <Button size="sm" fullWidth variant="outline" disabled className="text-xs h-10 border-orange-200 text-orange-500">
                          Menunggu Admin
                        </Button>
                    ) : (
                        <Button size="sm" fullWidth className="text-xs h-10" onClick={() => { setSelectedOrder(order); setModalType("tracking"); }}>
                          Lacak Pesanan
                        </Button>
                    )}
                  </>
      ...
      >
        {selectedOrder && (
          <PaymentProofModal 
            ...
            onSuccess={() => {
              setModalType(null);
              window.location.reload(); // Refresh halaman agar status berubah jadi 'verifikasi'
            }}
          />
        )}
      </BottomSheet>
